...
void overlay_error(int filename1,int filename2){
    char filepath1[100];
    char filepath2[100];
    ...
    auto graph1 = dynamic_cast<TGraphErrors*>(tf1->Get("Graph"));
    auto graph2 = dynamic_cast<TGraphErrors*>(tf2->Get("Graph"));
    ...
    legend->AddEntry( graph1, "double-slit open" , "pe") ; // AddEntry( pointer , "interpretation" , "option" )
    legend->AddEntry( graph2, "double-slit close" , "pe") ; // option は　"f"=box, "l"="L"=line, "p"=marker
